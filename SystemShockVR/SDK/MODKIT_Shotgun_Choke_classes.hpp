#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: MODKIT_Shotgun_Choke

#include "Basic.hpp"

#include "ITEM_WeaponModKit_Base_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass MODKIT_Shotgun_Choke.MODKIT_Shotgun_Choke_C
// 0x0000 (0x0238 - 0x0238)
class UMODKIT_Shotgun_Choke_C final : public UITEM_WeaponModKit_Base_C
{
public:
	void TryApplyWeaponModifications(class UITEM_WeaponBase_C* Weapon, bool DuringInitialization, bool* Result);
	void GetModifiedWeaponMode(const struct FSTRUCT_WeaponMode& WeaponMode, struct FSTRUCT_WeaponMode* ResultMode);
	void HasAppearanceChange(bool* Result);
	void GetFirstPersonAttachment(class UStaticMesh** Mesh, struct FTransform* Transform, class FName* SocketName);
	void TryModifyPickupAppearance(class APICKUP_Weapon_Base_C* WeaponPickup, bool* Result);

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"MODKIT_Shotgun_Choke_C">();
	}
	static class UMODKIT_Shotgun_Choke_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UMODKIT_Shotgun_Choke_C>();
	}
};
static_assert(alignof(UMODKIT_Shotgun_Choke_C) == 0x000008, "Wrong alignment on UMODKIT_Shotgun_Choke_C");
static_assert(sizeof(UMODKIT_Shotgun_Choke_C) == 0x000238, "Wrong size on UMODKIT_Shotgun_Choke_C");

}

